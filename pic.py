...
def RAP_SCORE_scatter():
    CH_path = os.path.join('./res/', '中文说唱.csv')
    EN_path = os.path.join('./res/', '英文说唱.csv')
    ...
    png_path = os.path.join('./res/', '中英文说唱网易评分散点.png')
...
def RAP_year_bar():
    ...
    fig.add_artist(legend_top)
    fig.subplots_adjust(right=0.82)
    fig.text(0.04, 0.5, 'Most popular rap publish num', va='center', rotation='vertical')
    plt.xticks(ind, yearlist,rotation = 45)

    png_path = os.path.join('./res', '最受欢迎的中英文说唱发行时间分布.png')
    plt.savefig(png_path)
...
def RAP_song_company_pie():
    ...
    plt.title('中英文说唱前十发行公司占比')
    plt.savefig('./res/中英文说唱前十发行公司占比.png')
    plt.show()
    print('Scatter Image saved: ', './res/中英文说唱前十发行公司占比.png')
    plt.close()
...
def RAP_singner_barH():
    ...
    plt.savefig('./res/中英文说唱前十歌手及上榜歌曲数目对比.png')
    plt.show()
    plt.close()
    print('Scatter Image saved: ', './res/中英文说唱前十歌手及上榜歌曲数目对比.png')

def RAP_sentiment_BAR():
    CH_path = os.path.join('./res/', '中文说唱-情感分类.csv')
    EN_path = os.path.join('./res/', '英文说唱-情感分类.csv')
    ...
    plt.ylabel('百分比')
    plt.title('中英文说唱情感分类对比')
    plt.xticks(ind, ('中文', '英文'))
    plt.yticks(np.arange(0, 110, 10))
    plt.legend((p1[0], p2[0], p3[0]), ('积极', '消极', '中性'))

    plt.savefig('./res/中英文说唱情感分析对比.png')
    plt.show()
    plt.close()
    print('Scatter Image saved: ', './res/中英文说唱情感分析对比.png')
#RAP_SCORE_scatter()
#RAP_year_bar()
